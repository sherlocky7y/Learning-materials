...
**数组元素**的作用与变量相当，一般来说凡是变量可以出现的地方都可以用数组进行代替。

**一维数组名作函数参数**

用数组名作函数参数需要在主调函数和被调用函数分别定义数组

> 与数组元素区别:
>
> 数组元素作实参时,向形参变量传递的是数组元素的值,
>
> 数组名作实参时,向形参(数组名或指针变量)传递的时数组首元素地址，所以形参数组可以不指定大小，在定义数组时在数组名后面跟一个空的方括号即可。

![image-20260809174521604](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260809174521604.png)
...
> 所谓选择法就是先将5个数中最小的数与a[0]对换；再将a[1]~a[9]中最小的数与a[1]对换，依次类推
...
int main(){
    system("chcp 65001 > nul"); 
    //这个只是用于避免中文乱码,引入<stdlib.h>主要就是引用system函数
    printf("请输入5个整数\n");
    int a[5];
    for(int i=0;i<5;i++){
        scanf("%d",&a[i]);
...
> 注意：可以使用多维数组作为函数的实参和形参，在被调用函数中对形参数组定义时，可以指定每一维的大小(可以省略第一维的大小)，但是不能把第2维以及其他高维大小说明省略
>
> 原因：二维数组是由若干一维数组组成，在内存中数组是按行存放的，因此在定义二维数组时，必须指定列数(即一行包含几个元素)，形参数组和实参数组类型相同，所以需要相同长度的一维数组组成。
...
> 有的函数可以被本文件中的其他函数调用，也可以被其他文件中的函数调用，而有的函数只能被本文件中的函数调用，不能被其他文件中的函数调用。

如果不加`声明`的话一个文件中的函数即可以被本文件中的其他函数调用，也可以被其他文件中函数调用。（但是可以指定某些函数不能被其他文件调用）

> 根据函数能否被其他源文件调用，将函数划分为**内部函数和外部函数**

##### 内部函数：

> 函数只能被本文件中的其他函数所调用

在定义函数时在函数名和函数类型前面加static

```c
static int fun(int a,int b)
```

> 表示fun是一个内部函数不能被其他文件调用

![image-20260811154721771](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260811154721771.png)

##### 外部函数：

> 定义函数时，在函数首部的最左端加关键字extern

```c
extern int fun(int a,int b)
```

> C语言规定，如果在定义函数时省略extern，默认是外部函数

调用须知：需要对调用的函数作声明，声明时需要加关键字extern（表示该函数是在其他文件中定义的外部函数）

P213有例题。

## 指针

> 想要理解指针，就必须先搞清楚数据是怎么在内存中进行存储的，又是如何读取的。
>
> 如果在程序中定义一个变量，在对程序进行编译时，系统会给这个变量分配内存单元。编译系统根据程序中定义的变量类型，分配一定长度的空间。内存区的每一个字节有一个编号（“地址”address），可以类比是旅馆的房间号。在地址所标志的内存单元中存放的数据可以类比是旅客。

由于通过地址能找到所需要的变量单元，可以说**地址指向变量单元**

> 一个房间门口挂着一个房间号2026，这个2026就是房间的地址（或者说2026指向该房间）

可以将地址形象化地称为`指针`(可以通过它找到以它为地址的内存单元)
...
- 间接访问
  ...
  先将变量a的地址存放到另一个变量中，然后通过该变量来找到变量a的地址，从而访问a变量。

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260811205502628.png" alt="image-20260811205502628" style="zoom: 25%;" />

> 讲解图中间接访问：将3送到变量i_pointer所指向的单元（变量i的存储单元）→\*i_pointer=3;(其中\*i_pointer表示i_pointer指向的对像)

**指向**就是通过**地址**来体现的

![image-20260811210241471](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260811210241471.png)
...
**指针变量**：也是一种变量作用就是用来存放地址（地址只能用地址符"&"得到并赋值给一个指针变量）

```c
p=&a
```

> 定义指针变量左侧要有类型名（用来指定此指针变量可以指向的变量的类型，且不同类型的数据在内存中所占的字节数和存放）
>
> 所以指针变量是基本数据类型派生出来的类型，**不能离开基本类型而独立存在**
...
| ---- | -------------------- | -------------------------- |
| &a   | 获取变量 a 的地址    | "a 住在哪"                 |
| *pa  | 根据pa获取地址里的值 | "去这个地址看看里面有什么" |

### 指针含义

- 一是存储单元编号表示的纯地址
- 一是它指向的存储单元的数据类型（int float char等）

"int *"读作：**”指向int的指针“**或者简称**”int指针“**
...
//引用指针变量指向的变量
*p=1    //在经历上面赋值之后，现在把1赋值p所指向的变量，本质上相当于把1赋值给a

//引用指针变量的值
printf("%o",p)
//以八进制数形式输出指针变量p的值，其实就是&a，a的地址
```

### 指针的移动和指针的运算

> 如果指针是指向一个整型变量的，那么“使指针移动1个位置”意味着移动4个字节，使“指针加1”意味着使地址值加4个字节，
>
> 如果是字符变量就变成加1字节（所以必须指明基类型）
...
> 函数的参数不仅可以是整型，浮点型，字符型等，还可以是指针类型
>
...
![image-20260818211323442](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260818211323442.png)

> 不可能通过执行调用函数来改变实参指针变量的值，但是可以改变实参指针变量所指变量的值

注意：函数调用可以且只可以得到一个返回值（即函数值），而使用指针变量作为参数，可以得到多个变化了的值。（充分发挥指针优势）

### 指针和数组

> 一个变量有地址，一个数组包含若干元素，每个数组元素都在内春中占用存储单元，都有相应的地址。
>
> 指针变量可以指向变量当然也可以指向数组元素，所谓数组元素指针就是数组元素地址。
...
> 引用数组可以用下标法（eg:a[3]）,也可以用指针法，即通过指向数组的指针找到所需元素（优点：占内存少，运行速度快）
>
> 注意：在C语言中，数组名（不包含形参数组名）代表数组中首元素(即序号为0的元素)的地址

```c
p=&a[0];
//等价
p=a;   //p的值是数组a首元素，程序中数组名不代表整个数组，只代表数组首元素地址。
```
...
> 什么情况下需要用指针型数据的算数运算呢？含义是什么？

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260819165432386.png" alt="image-20260819165432386"  />

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260819165517777.png" alt="image-20260819165517777"  />

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260819165501399.png" alt="image-20260819165501399"  />

![image-20260819165550667](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260819165550667.png)
...
![image-20260823012309687](https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260823012309687.png)

> 指针变量指向某一数组元素，但是实际上指针变量可以指向数组以后的存储单元，如果在程序中引用不存在的数组元素，C语言程序并不会认为它非法。只是与预期结果不同。

**用数组名作函数参数**

> 书本介绍：

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260824130318778.png" alt="image-20260824130318778" style="zoom: 67%;" />

>C语言调用函数时虚实结合的方法都是“值传递”
>
>变量名作为函数参数时传递的是变量的值
>
>数组名作为函数参数时（由于数组名代表的是数组首元素地址），传递的是值是地址，所以要求形参为指针变量
>
>用数组名作函数参数形参本质就是一个指针变量。

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260824130435765.png" alt="image-20260824130435765" style="zoom: 50%;" />

> 为什么还可以是形参数组的形式呢？
>
> 在C语言中下标法和指针法都可以访问一个数组

注意：设定函数形参数组可以不指定大小，因为形参数组名实际上是一个指针变量，并不是真正去开辟数组空间。、

**通过指针引用多维数组**

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260829132114051.png" alt="image-20260829132114051" style="zoom: 67%;" />

<img src="https://raw.githubusercontent.com/sherlocky7y/blog-images/main/imgs/image-20260829132123961.png" alt="image-20260829132123961" style="zoom: 50%;" />
